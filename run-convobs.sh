...
ruby ${bindir}/convobs-diff.rb --unify convgts-${ymd}.txt convwis-${ymd}.txt > onlygts-${ymd}.txt
ruby ${bindir}/convobs-diff.rb --unify convwis-${ymd}.txt convgts-${ymd}.txt > onlywis-${ymd}.txt
m1=/nwp/m1/convobs-cur.txt 
if test -f ${m1} ; then
ruby ${bindir}/convobs-diff.rb --unify ${m1} convwis-${ymd}.txt > onlym1-${ymd}.txt
ruby ${bindir}/convobs-diff.rb --unify convwis-${ymd}.txt ${m1} > onlym2-${ymd}.txt
fi
